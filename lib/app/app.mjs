/* TurboSign 4.0 — ESM base + Safari double‑tap zoom suppression (safe)
   - Keeps custom double‑tap (Text/Signature) working
   - Prevents iOS Safari page zoom on double‑tap (toolbar + stage)
   - PDF.js v5 ESM + module worker (Blob fallback w/ plain string)
   - pdf-lib ESM (+esm)
*/
const DBG = (...a)=>console.debug(`[${new Date().toLocaleTimeString()}]`, ...a);
...
/* ---------- PDF.js (ESM) ---------- */
const PDFJS_VER = '5.4.54';
const CORE_URL   = `https://cdn.jsdelivr.net/npm/pdfjs-dist@${PDFJS_VER}/build/pdf.mjs`;
const WORKER_URL = `https://cdn.jsdelivr.net/npm/pdfjs-dist@${PDFJS_VER}/build/pdf.worker.mjs`;
let pdfjsLib;

try {
  pdfjsLib = await import(CORE_URL);
  DBG('PDF.js core loaded');
} catch (e) {
  console.error(e);
  toast('Could not load PDF engine core', 'err', 4000);
  throw e;
}

try {
  pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(WORKER_URL, { type:'module' });
  DBG('PDF.js worker: module OK (direct)');
} catch (e) {
  DBG('PDF.js worker direct failed:', e?.message || e);
  try {
    const workerCode = 'import "' + WORKER_URL + '";';   // plain string (Safari OK)
    const blob = new Blob([workerCode], { type: 'text/javascript' });
    const blobURL = URL.createObjectURL(blob);
    pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(blobURL, { type:'module' });
    DBG('PDF.js worker: module OK via Blob');
  } catch (e2) {
    DBG('PDF.js worker FAILED; main thread', e2?.message || e2);
    pdfjsLib.GlobalWorkerOptions.workerPort = null; // fallback
  }
}
...
/* ---------- Pinch/trackpad zoom + block Safari dbl‑tap zoom ---------- */
(function wirePinch(){
  ...
  // Block iOS native gesture within stage
  ...
  // Suppress Safari double‑tap zoom INSIDE the stage without killing our own dbl‑tap:
  // We prevent default only on generic touches; our overlay dbl‑tap handler calls preventDefault itself.
  let lastTouchEnd = 0;
  refs.scroll.addEventListener('touchend', (e)=>{
    const now = Date.now();
    if (now - lastTouchEnd < 300) { e.preventDefault(); } // stop page zoom
    lastTouchEnd = now;
  }, {passive:false});
})();
...
class Annotations{
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach(({wrap})=>{
      ...
      // Touch double‑tap (our own) — IMPORTANT: passive:false + preventDefault to stop Safari zoom
      if(isTouch){
        let t=0,lx=0,ly=0;
        on(ov,'pointerdown',e=>{
          ...
          t=now; lx=x; ly=y;
          if(!dbl) return;
          e.preventDefault(); // stop Safari dbl‑tap page zoom
...
class SigPad{
  constructor(canvas, modal){
    this.canvas=canvas; this.modal=modal; this.ctx=canvas.getContext('2d'); this.clear(); this.drawing=false;
...
/* ---------- UI wiring ---------- */
function wireUI(){
  on(refs.openBtn,'click',()=>{ DBG('OPEN click'); refs.fileInput?.click(); });
  on(refs.fileInput,'change',async e=>{
    const file=e.target.files?.[0]; if(!file) return;
    DBG('OPEN picked', file.name, file.type, file.size);
  ...
  on(refs.textBtn,'click',()=>{ DBG('TEXT click'); ann.setMode(ann.mode==='text'?null:'text'); toast(ann.mode?(isTouch?'Text: double‑tap':'Text: double‑click'):'Tool off'); });
  on(refs.stampBtn,'click',()=>{ DBG('STAMP click'); ann.setMode(ann.mode==='stamp'?null:'stamp'); toast(ann.mode?'Stamp: tap/click':'Tool off'); });
  on(refs.signBtn,'click', ()=> { DBG('SIGN click (open modal)'); sigCtl.open(); });
  on(refs.undoBtn,'click',()=>{ DBG('UNDO click'); ann.undo(); });
  on(refs.redoBtn,'click',()=>{ DBG('REDO click'); ann.redo(); });
  on(refs.helpBtn,'click',()=>{ DBG('HELP click'); toast('📂 Open → double‑tap Text/Signature (draw first) → Save. Drag to move.','ok',4200); });
  on(refs.saveBtn,'click',()=>{ DBG('SAVE click'); saveFlattened(); });

  on(refs.sigUse,'click',e=>{ e.stopPropagation(); DBG('SIG use'); sigCtl.use(); });
  on(refs.sigClear,'click',e=>{ e.stopPropagation(); DBG('SIG clear'); sig.clear(); });
  on(refs.sigCancel,'click',e=>{ e.stopPropagation(); DBG('SIG cancel'); sigCtl.close(); });
}
wireUI();
