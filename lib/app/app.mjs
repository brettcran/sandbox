/* TurboSign 4.0 + thumbnails rail + right-side scroll thumb + X-lock + fit-width
   ESM-only, no codebase restructure. */
...
try {
  ...
  console.warn('[PDF] worker failed, main thread fallback', e);
...
const refs = {
  ...
  stack: $('#pdf-stack'),
  thumbs: $('#thumbs'),
  thumbsToggle: $('#thumbs-toggle'),
  vscroll: $('#vscroll'), vthumb: $('#vthumb'),
...
let CURRENT_PDF = { file:null, bytes:null, filename:null, wraps:[], vpCSSByPage:[], scaleBase:1 };
let LAST_SIG_DATAURL = null;

/* --------- zoom (scale target is #pdf-stack) --------- */
const zoom = {
  scale: 1, min: 0.9, max: 2.5, suspended:false,  // tighter zoom range per your ask
  setScale(newScale, cx, cy){
    ...
    const stack = refs.stack;

    const contentW0 = stack.scrollWidth * this.scale;
    const contentH0 = stack.scrollHeight * this.scale;
    const contentW1 = stack.scrollWidth * newScale;
    const contentH1 = stack.scrollHeight * newScale;
    ...
    stack.style.transform = `scale(${this.scale})`;

    // hard X lock (container already overflow-x:hidden) — always center
    scroll.scrollLeft = 0;
...
(function wirePinch(){
  ...
  refs.scroll.addEventListener('wheel', (e)=>{ if(!(e.ctrlKey||e.metaKey)) return; e.preventDefault(); zoom.setScale(zoom.scale*(e.deltaY<0?1.08:0.92), e.clientX, e.clientY); }, {passive:false});
...
/* center helper (fit-width on open) */
function fitWidthAndCenter(){
  const firstWrap = CURRENT_PDF.wraps[0];
  if(!firstWrap) return;
  const canvas = firstWrap.querySelector('canvas');
  if(!canvas) return;
  const natural = canvas.getBoundingClientRect().width / zoom.scale; // current CSS width / scale
  const avail = refs.container.clientWidth - (refs.thumbs.classList.contains('open') ? 96+16 : 0);
  const targetScale = clamp(avail / natural, zoom.min, zoom.max);
  zoom.setScale(targetScale, refs.scroll.clientWidth/2, 40);
...
class Annotations{
  ...
  _onUp(e){
    ...
    if(changed){ /* could mark session dirty here */ }
  }
  ...
  _select(el){ refs.stack.querySelectorAll('.anno').forEach(n=>n.classList.remove('selected')); this.selected=el; if(el) el.classList.add('selected'); }
...
const ann = new Annotations(refs.stack);

function scrollAnnoIntoView(el){
  if (!el) return;
  const wrap = el.closest('.page-wrap'); if (!wrap) return;
  const stackR = refs.stack.getBoundingClientRect();
  const wrapR = wrap.getBoundingClientRect();
  const elCY = (wrapR.top - stackR.top + el.offsetTop + el.offsetHeight/2) * zoom.scale;
  const targetTop  = clamp(elCY - refs.scroll.clientHeight/2, 0, refs.scroll.scrollHeight);
  refs.scroll.scrollTo({ top: targetTop, behavior: 'smooth' });
}
...
/* --------- render + thumbnails + scroll thumb --------- */
...
async function renderPdfFromData(bytes, scale=1){
  refs.stack.innerHTML=''; refs.thumbs.innerHTML=''; CURRENT_PDF.wraps=[]; CURRENT_PDF.vpCSSByPage=[];
  let pdf;
  try { pdf = await pdfjsLib.getDocument({ data: bytes }).promise; }
  catch(err){ console.error('getDocument failed', err); toast('Could not open PDF (engine error)','err', 3200); throw err; }
  ...
  for(let i=1;i<=pdf.numPages;i++){
    ...
    // overlay
    // (attached later via ann.attachOverlays after loop)
    // thumbnails
    const tWrap=document.createElement('div'); tWrap.className='pg'; tWrap.dataset.index=String(i-1);
    const tCanvas=document.createElement('canvas'); const tScale=80/vpCSS.width; const tvp=page.getViewport({scale:tScale});
    tCanvas.width=tvp.width; tCanvas.height=tvp.height; const tctx=tCanvas.getContext('2d'); await page.render({canvasContext:tctx, viewport:tvp}).promise;
    tWrap.appendChild(tCanvas); refs.thumbs.appendChild(tWrap);
    on(tWrap,'click',()=>{ jumpToPage(i-1,true); });
  }

  ann.attachOverlays(CURRENT_PDF.wraps);

  // show custom v-scroll
  refs.vscroll.style.display='block';

  requestAnimationFrame(()=>{ fitWidthAndCenter(); highlightThumb(0); updateVThumb(); });
  setTimeout(()=>{ fitWidthAndCenter(); updateVThumb(); }, 100);
}

/* page jump + thumb highlight */
function pageTop(i){
  const wrap = CURRENT_PDF.wraps[i];
  if(!wrap) return 0;
  const y = wrap.offsetTop * zoom.scale;
  return y;
}
function jumpToPage(i, smooth){
  const top = pageTop(i);
  refs.scroll.scrollTo({ top, behavior: smooth ? 'smooth' : 'auto' });
  highlightThumb(i);
  updateVThumb();
}
function highlightThumb(i){
  refs.thumbs.querySelectorAll('.pg').forEach((n,idx)=>n.classList.toggle('active', idx===i));
}

/* right-side custom scroll thumb */
function updateVThumb(){
  const rail = refs.vscroll, thumb = refs.vthumb;
  const s = refs.scroll;
  const content = refs.stack.scrollHeight * zoom.scale;
  const view = s.clientHeight;
  if (content <= view){ thumb.style.height = '0px'; thumb.style.top='0px'; return; }
  const railH = rail.clientHeight;
  const thumbH = Math.max(40, railH * (view / content));
  const maxTop = railH - thumbH;
  const ratio = s.scrollTop / (content - view);
  thumb.style.height = `${Math.round(thumbH)}px`;
  thumb.style.top = `${Math.round(maxTop * ratio)}px`;
}
(function wireVThumb(){
  let drag=false, startY=0, startTop=0, railH=0, thumbH=0, maxTop=0;
  const rail = refs.vscroll, thumb = refs.vthumb, s = refs.scroll;

  const start = (e)=>{ drag=true; thumb.style.cursor='grabbing'; startY=e.clientY; railH=rail.clientHeight; thumbH=thumb.clientHeight; maxTop=railH-thumbH; startTop=parseFloat(getComputedStyle(thumb).top)||0; e.preventDefault(); };
  const move  = (e)=>{ if(!drag) return; let dy=e.clientY-startY; let nt=clamp(startTop+dy,0,maxTop);
    thumb.style.top=nt+'px';
    const content = refs.stack.scrollHeight * zoom.scale;
    const view = s.clientHeight;
    const ratio = nt/maxTop;
    const newScrollTop = ratio * (content - view);
    s.scrollTop = newScrollTop;
  };
  const end   = ()=>{ drag=false; thumb.style.cursor='grab'; };

  on(thumb,'pointerdown',start,{passive:false});
  on(window,'pointermove',move,{passive:false});
  on(window,'pointerup',end);

  on(refs.scroll,'scroll',()=>{ updateVThumb();
    // auto-highlight current thumb by middle-of-view page
    let centerY = refs.scroll.scrollTop + refs.scroll.clientHeight/2;
    let idx = CURRENT_PDF.wraps.findIndex(w=> (w.offsetTop*zoom.scale) > centerY );
    idx = (idx<=0?0:idx-1);
    highlightThumb(idx);
  });
})();
...
/* --------- UI wiring (same controls) --------- */
function wireUI(){
  on(refs.thumbsToggle,'click',()=>{
    refs.thumbs.classList.toggle('open');
    requestAnimationFrame(()=>{ fitWidthAndCenter(); });
  });

  on(refs.openBtn,'click',()=>refs.fileInput?.click());
  on(refs.fileInput,'change',async e=>{
    const file=e.target.files?.[0]; if(!file) return;
    try { await renderPdfFromFile(file, 1); toast('PDF loaded ✔️'); }
  ...
  on(refs.helpBtn,'click',()=>toast('📂 Open → thumbnails toggle (Pages) → double‑tap Text/Signature (draw first) → Save. Drag to move.','ok',4200));
  ...
  on(window,'resize',()=>{ fitWidthAndCenter(); updateVThumb(); });
}
wireUI();
