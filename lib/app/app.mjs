/* TurboSign 4.0 — Center-on-open fix (rock-solid), same ESM stack, same tools.
   - PDFs open at 1.0 scale, horizontally centered (and top vertically)
   - Robust centering on initial render, after layout, after resize/orientation
   - Pinch/wheel zoom preserved; annotation placement unchanged
*/
...
/* ---------- PDF.js worker (module + Blob fallback for Safari) ---------- */
...
/* This centers the scaled content within the scroll viewport.
   Called:
   - after rendering all pages
   - after layout settles (via rAF+micro delay)
   - on resize and orientation changes
*/
function centerContent({x='center', y='top', behavior='auto'} = {}){
  const scroll = refs.scroll, cont = refs.container;
  if (!scroll || !cont) return;

  const contentW = cont.scrollWidth * zoom.scale;
  const contentH = cont.scrollHeight * zoom.scale;
  ...
  if (x === 'center'){
    targetLeft = Math.max(0, (contentW - viewW) / 2);
  } else if (typeof x === 'number'){
    targetLeft = clamp(x, 0, Math.max(0, contentW - viewW));
  }

  if (y === 'center'){
    targetTop = Math.max(0, (contentH - viewH) / 2);
  } else if (y === 'top'){
    targetTop = 0;
  } else if (typeof y === 'number'){
    targetTop = clamp(y, 0, Math.max(0, contentH - viewH));
  }

  // If content smaller than viewport, natural CSS centering applies (scroll 0)
  if (contentW <= viewW) targetLeft = 0;
  if (contentH <= viewH && y !== 'number') targetTop = 0;

  scroll.scrollTo({ left: targetLeft, top: targetTop, behavior });
}

// Debounced center on resize/orientation
let _resizeTimer = null;
function scheduleCenter(){
  clearTimeout(_resizeTimer);
  _resizeTimer = setTimeout(()=>centerContent({x:'center', y:'top'}), 50);
}
on(window,'resize', scheduleCenter);
on(window,'orientationchange', scheduleCenter);
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  // Reset scale to exactly 1 and center strongly after layout
  zoom.scale = 1;
  refs.container.style.transformOrigin = '0 0';
  refs.container.style.transform = 'scale(1)';

  // Strong centering sequence (works across Safari/Chrome/Edge/Firefox)
  centerContent({x:'center', y:'top', behavior:'auto'});
  requestAnimationFrame(()=>centerContent({x:'center', y:'top', behavior:'auto'}));
  setTimeout(()=>centerContent({x:'center', y:'top', behavior:'auto'}), 80);
...
function wireUI(){
  on(refs.fileInput,'change',async e=>{
    ...
    catch(err){ toast('Could not open PDF','err'); }
  });
