/* TurboSign 4.0 — CDN-only ESM build (delegated toolbar wiring + debug console)
   - PDF.js v5 via CDN ESM + module worker (fallback to main thread)
   - pdf-lib via CDN ESM (+esm)
   - Same UX as your working 4.0
   - NEW: Toolbar uses event delegation so every button works uniformly on all browsers
*/
...
(() => {
  try {
    ...
    Debug.log('PDF.js worker: module OK');
  } catch (e) {
    pdfjsLib.GlobalWorkerOptions.workerPort = null;
    Debug.log('PDF.js worker FAILED; main thread', String(e?.message||e));
  }
})();
...
/* ========== Toolbar event delegation (rock‑solid) ========== */
function wireToolbar(){
  // Long-press on Help opens debug
  let helpTimer=null;
  on(refs.toolbar, 'pointerdown', (e)=>{
    const btn = e.target.closest('.btn'); if (!btn) return;
    const act = btn.dataset.act;
    if (act==='help'){ helpTimer=setTimeout(()=>{ Debug.toggle(); helpTimer=null; }, 500); }
  }, {passive:true});
  on(refs.toolbar, 'pointerup', ()=>{
  ...
  refs.toolbar.addEventListener('click', async (e)=>{
    ...
    switch (act) {
      case 'open': {
        ...
        }catch(err){ console.warn('Open picker failed', err); }
        break;
      }
      case 'text': {
      ...
      case 'sign': {
        // Always open modal to (re)draw; placement happens on double‑tap/click afterward
        sigCtl.open();
        // While a signature exists, treat “Sign” as armed state visually
      ...
      case 'help': {
        // If long-press already opened debug, do nothing; else show hint
        if (helpTimer){ toast('📂 Open → double‑tap Text or Signature (draw first) → Save. Drag to move.','ok',4200); }
        break;
      }
      default: break;
    }
