/* TurboSign 4.0 — Stability patch
   - Fix long-press highlight on toolbar (iOS)
   - Keep text selection only inside text annotations
   - Robust PDF.js ESM worker (module/Blob)
   - Preserve all 4.0 features + collapsible page thumbnails
*/

const CDN_PDFJS = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/';

import * as pdfjsLib from `${CDN_PDFJS}pdf.mjs`;

(async () => {
  try {
    try {
      pdfjsLib.GlobalWorkerOptions.workerPort =
        new Worker(`${CDN_PDFJS}pdf.worker.mjs`, { type:'module' });
      console.log('[worker] module direct OK');
    } catch (e) {
      console.warn('[worker] direct failed:', e);
      const blob = new Blob([`import "${CDN_PDFJS}pdf.worker.mjs";`], { type: 'text/javascript' });
      const url  = URL.createObjectURL(blob);
      pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(url, { type:'module' });
      console.log('[worker] module via Blob OK');
    }
  } catch (err) {
    console.error('[worker] fallback to main thread', err);
...
const refs = {
  ...
  pageToggle: $('#page-toggle'), pageNav: $('#page-nav'),
};
...
/* Prevent context menu/selection on chrome/safari long-press outside editable areas */
document.addEventListener('contextmenu', (e)=>{
  const withinToolbar = e.target.closest?.('#toolbar, #page-toggle, #page-nav, #restore-banner');
  const isEditable    = e.target.closest?.('.anno.text, [contenteditable="true"]');
  if (withinToolbar && !isEditable) e.preventDefault();
}, {capture:true});

/* Keep text inputs usable: toggle user-select when focusing text annotations */
function allowTextSelectionInside(el, allow){
  if (!el) return;
  if (allow) {
...
const zoom = {
  scale: 1, min: 0.8, max: 2.5, suspended:false,
...
// pointer pinch with Safari gesture guards
(function wirePinch(){
  ...
  const inStage = el => !!el && (el===refs.scroll || el===refs.container || el.closest?.('#pdf-stage, #pdf-scroll, #pdf-container'));
  addEventListener('gesturestart',  e=>{ if(inStage(e.target)) e.preventDefault(); }, {passive:false});
  addEventListener('gesturechange', e=>{ if(inStage(e.target)) e.preventDefault(); }, {passive:false});
  addEventListener('gestureend',    e=>{ if(inStage(e.target)) e.preventDefault(); }, {passive:false});
...
class Annotations{
  ...
  _addText(overlay,x,y,focus){
    ...
    overlay.appendChild(el); this._wireAnno(el, overlay, {resizable:false}); if(focus){ this._focus(el); allowTextSelectionInside(el,true); }
  ...
  _wireAnno(el, overlay, {resizable}){
    const startDrag=(e)=>{
      ...
      this.drag={ el, overlay, dx:x-left, dy:y-top }; $('#pdf-scroll').style.touchAction='none'; zoom.suspended=true;
    ...
    if(el.classList.contains('text')){
      on(el,'dblclick',()=>{ this._focus(el); this._select(el); allowTextSelectionInside(el,true); });
      on(el,'blur',()=>{ allowTextSelectionInside(el,false); });
      on(el,'input',()=>{ /* dirty snapshot if needed */ });
    }
  ...
  _onUp(e){
    ...
    if(changed) { /* mark dirty if snapshotting */ }
  }
...
/* ---------- page thumbnails (collapsible panel) ---------- */
function buildThumb(container, page, index){
  const wrap = document.createElement('div'); wrap.className='thumb';
  const cnv  = document.createElement('canvas'); cnv.width = 200; cnv.height = 260;
  const tag  = document.createElement('div'); tag.className='pg'; tag.textContent = String(index+1);
  wrap.appendChild(cnv); wrap.appendChild(tag); container.appendChild(wrap);
  const vp = page.getViewport({ scale: 0.25 });
  const ratio = Math.max(1, Math.min(2, devicePixelRatio||1));
  cnv.width  = Math.floor(vp.width * ratio);
  cnv.height = Math.floor(vp.height * ratio);
  cnv.style.width = Math.floor(vp.width) + 'px';
  cnv.style.height= Math.floor(vp.height) + 'px';
  const ctx = cnv.getContext('2d', {alpha:false});
  page.render({ canvasContext: ctx, viewport: page.getViewport({scale:0.25*ratio}), intent:'display' });
  return wrap;
}
function scrollToPage(index){
  const wrap = CURRENT_PDF.wraps[index]; if(!wrap) return;
  const top = wrap.offsetTop * zoom.scale;
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  refs.pageNav.innerHTML=''; // clear thumbnails
  let pdf;
  ...
  for(let i=1;i<=pdf.numPages;i++){
    ...
    const t = buildThumb(refs.pageNav, page, i-1);
    t.addEventListener('click', ()=>{ scrollToPage(i-1); });
  }
...
function wireUI(){
  ...
  const toggle = ()=>{
    const show = !refs.pageNav.classList.contains('show');
    refs.pageNav.classList.toggle('show', show);
    refs.pageToggle.setAttribute('aria-expanded', String(show));
  };
  on(refs.pageToggle,'click', toggle);
}
wireUI();
