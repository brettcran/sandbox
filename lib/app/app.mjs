/* TurboSign 4.0 — ESM + Blob worker fallback + gentler zoom + calibrated annotations */
...
const Debug = (() => {
  ...
  function ensure(){ if(!panel) build(); }
  function fmt(x){try{return typeof x==='string'?x:JSON.stringify(x);}catch{return String(x);}}
  function log(...args){ ensure(); const d=document.createElement('div'); const ts=new Date().toISOString().split('T')[1].replace('Z',''); d.textContent=`[${ts}] ${args.map(fmt).join(' ')}`; list.appendChild(d); if(list.children.length>400) list.removeChild(list.firstChild); list.scrollTop=list.scrollHeight; console.log('[TS]',...args); }
  function show(on){ ensure(); panel.style.display=on?'block':'none'; vis=on; }
  function toggle(){ show(!vis); }
  return { log, show, toggle };
})();
...
/* ---------- PDF.js worker (Blob fallback for Safari) ---------- */
...
/* ---------- zoom (gentler; center-kept) ---------- */
const zoom = {
  scale: 1, min: 0.8, max: 2.6, suspended:false,
...
(function wirePinch(){
  ...
  const onPM = (e)=>{ if(!pts.has(e.pointerId)) return; pts.set(e.pointerId,e);
    if(pinching && pts.size===2){ const [a,b]=[...pts.values()]; const d=Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY);
      ...
      if(lastDist){ const factor=(d/lastDist) ** 0.85; zoom.setScale(zoom.scale*factor, cx, cy); } // gentler
  ...
  refs.scroll.addEventListener('wheel', (e)=>{ if(!(e.ctrlKey||e.metaKey)) return; e.preventDefault();
    const factor = (e.deltaY < 0) ? 1.06 : 0.94; // gentler
...
class Annotations{
    ...
    wrapInfos.forEach(({wrap})=>{
      ...
      on(ov,'dblclick', e=>{
        ...
        if (this.mode==='text'){
          ...
          const el=this._addSignature(ov,x,y,LAST_SIG_DATAURL, ov.clientWidth*0.4, /*center*/true);
          this._select(el); scrollAnnoIntoView(el);
        }
      });

      // Robust touch double‑tap detector
      if(isTouch){
        let lastT=0, lastX=0, lastY=0;
        on(ov,'pointerdown', e=>{
          if (e.pointerType!=='touch' || e.target!==ov) return;
          const now=performance.now(); const {x,y}=this._pos(ov,e);
          const isDbl = (now-lastT<300 && Math.abs(x-lastX)<24 && Math.abs(y-lastY)<24);
      ...
      // Stamp: single tap/click (on release) to avoid conflicts with pinch
      on(ov,'pointerup', e=>{
        if(this.mode!=='stamp' || e.target!==ov || zoom.suspended) return;
        const {x,y}=this._pos(ov,e); this._addStamp(ov,x,y, /*center*/true);
      }, {passive:true});
  ...
  _addStamp(overlay,x,y,center=false){
    const el=document.createElement('div'); el.className='anno stamp';
    // default SVG size; will center after insert
    el.innerHTML=`<svg viewBox="0 0 24 24" width="28" height="28"><path d="M4 12l4 4 12-12"/></svg>`;
    overlay.appendChild(el);
    // measure then place
    const { w, h } = this._elSize(el);
    el.style.left=`${Math.max(0, x-(center?w/2:0))}px`;
    el.style.top =`${Math.max(0, y-(center?h/2:0))}px`;
    this._wireAnno(el, overlay, {resizable:false});
    this._recordAdd(overlay, el);
  }

  _addSignature(overlay,x,y,dataURL,widthHint,center=false){
    ...
    img.onload=()=>{
      ...
      const { w, h } = this._elSize(el);
      // center at tap
      let nx = Math.max(0, Math.min(x - (center?w/2:0), overlay.clientWidth  - w));
      let ny = Math.max(0, Math.min(y - (center?h/2:0), overlay.clientHeight - h));
  ...
  _wireAnno(el, overlay, {resizable}){
    ...
    if(el.classList.contains('text')){
      ...
      on(el,'input',()=>{ /* mark dirty if you add snapshots later */ });
    }
  ...
  _onUp(e){
    ...
    if(changed){ /* mark dirty if session snapshot returns */ }
  }
...
/* ---------- save (same as last stable w/ diagnostics) ---------- */
async function saveFlattened(){
  ...
  }catch(e){ Debug.log('Rehydrate failed:', e?.message||e); }
  let pdf, helv, helvB;
  try{
  ...
  }catch(e){ Debug.log('PDFDocument.load failed:', e?.message||e); toast('Save failed: cannot read original PDF','err', 3600); return; }
  ...
  try{
          ...
          const p = (async ()=>{
            try{
              ...
              if (img.src.startsWith('data:')){
                const comma = img.src.indexOf(','); const b64 = comma>0 ? img.src.slice(comma+1) : '';
                const bin = atob(b64); bytes=new Uint8Array(bin.length); for(let i=0;i<bytes.length;i++) bytes[i]=bin.charCodeAt(i);
            ...
            }catch(err){ Debug.log('Signature embed failed:', err?.message||err); }
          })();
          embedOps.push(p);
        }
      });
    ...
    toast('Saved ✔️');
  }catch(e){ Debug.log('Save failed:', e?.message||e); toast('Could not save PDF', 'err', 3600); }
}
