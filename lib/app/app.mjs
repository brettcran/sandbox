/* TurboSign 4.0 + Page Navigator (collapsible, right) — ESM only
   - Keeps your working 4.0 behaviors
   - Locks PDF.js worker to ESM Blob worker (stable on Safari)
   - Adds right-side, collapsible thumbnail panel (never overlaps toolbar)
*/
...
// ---- lock worker: ESM Blob worker (works on Safari) ----
(async () => {
  try {
    // Try direct module worker first (may throw on iOS)
    try {
      pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(`${CDN_PDFJS}pdf.worker.mjs`, { type:'module' });
...
/* ---------- annotations (unchanged from 4.0) ---------- */
class Annotations{
  ...
  _onUp(e){
    ...
    if(changed) { /* could mark dirty for snapshots */ }
  }
...
function buildThumb(container, page, index){
  ...
  const cnv  = document.createElement('canvas'); cnv.width = 200; cnv.height = 260; // will be resized
