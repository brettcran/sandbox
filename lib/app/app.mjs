/* TurboSign 4.0 — CDN-only ESM build (delegated toolbar wiring + debug console + Blob worker fallback)
   - PDF.js v5 via CDN ESM + module worker (Blob fallback for Safari “operation is insecure”)
...
async function bootPdfWorker(){
  // 1) Try direct module worker from CDN
  try {
  ...
  // 2) Blob module worker (same-origin) – fixes Safari “operation is insecure”
  try {
