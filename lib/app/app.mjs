/* TurboSign 4.0 — stable ESM base with DEBUG
   - PDF.js v5 ESM + module worker (with Blob fallback that avoids nested template strings)
   - pdf-lib ESM (+esm)
   - Minimal UI logic (text / stamp / signature), undo/redo, save flatten
   - Centered pinch zoom logic preserved
*/
...
// Try direct module worker; if Safari blocks it (security/CSP), fall back to Blob URL
try {
  ...
  try {
    // IMPORTANT: plain string (no nested template literal), Safari-friendly
    const workerCode = 'import "' + WORKER_URL + '";';
...
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent ink on white pad (pad looks white, export is transparent)
}
