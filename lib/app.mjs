/* TurboSign 4.0 — ESM PWA (Open fix: label trigger + resilient fallback) */

import * as pdfjsLib from '../build/pdf.mjs';
const LOCAL_WORKER = new URL('../build/pdf.worker.mjs', import.meta.url).href;

try {
  const w = new Worker(LOCAL_WORKER, { type: 'module' });
  pdfjsLib.GlobalWorkerOptions.workerPort = w;
} catch (e) {
  console.warn('[PDF.js] Local module worker failed → main thread', e);
...
const refs = {
  ...
  openProxy: $('#open-proxy'), // <label for="file-input">
...
/* ---------- annotations (same behavior as 4.0) ---------- */
class Annotations{
  ...
  _onUp(e){
    ...
    if(changed) {/* dirty mark optional */}
  }
...
async function renderPdfFromData(bytes, container, scale=1){
  if (!isValidPdfBytes(bytes)) { toast('That file is not a valid PDF (missing %PDF header).','err',3600); throw new Error('Invalid PDF header'); }

  container.innerHTML=''; CURRENT_PDF.wraps=[]; CURRENT_PDF.vpCSSByPage=[];
  let pdf;
  try { pdf = await pdfjsLib.getDocument({ data: bytes }).promise; }
  catch(err){ console.error('getDocument failed', err); toast('Could not open PDF','err', 3600); throw err; }
...
/* ---------- UI wiring ---------- */
function wireUI(){
  // Primary: label#open-proxy handles the picker natively (no JS needed)
  // Fallbacks (in case someone triggers programmatically or the label can't be used)
  on(refs.openProxy, 'click', ()=>{/* no-op: native label dispatches click to input */});

  // Extra resilience: also support programmatic open if needed
  function openPickerFallback(){
    try{
      if (refs.fileInput.showPicker) refs.fileInput.showPicker();
      else refs.fileInput.click();
    }catch(e){
      // Ultra fallback: temporarily make it visible and focus it
      refs.fileInput.style.opacity='0.001';
      refs.fileInput.style.position='fixed';
      refs.fileInput.style.left='10px';
      refs.fileInput.style.top='10px';
      refs.fileInput.click();
      setTimeout(()=>{
        refs.fileInput.style.position='absolute';
        refs.fileInput.style.left='-9999px';
        refs.fileInput.style.top='0';
        refs.fileInput.style.opacity='0';
      }, 600);
    }
  }

  // Keep a keyboard shortcut, too (Cmd/Ctrl+O)
  addEventListener('keydown', (e)=>{
    if ((e.ctrlKey||e.metaKey) && (e.key==='o' || e.key==='O')) {
      e.preventDefault(); openPickerFallback();
    }
  });
